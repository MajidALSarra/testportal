...
const input = {
  ...
  moveVec: { x: 0, y: 0 },      // resolved movement vector (from touch move stick)
  aimVec: { x: 0, y: 0 }, aimActive: false,
  // Floating twin-stick touch state. Each is null or {id, ox, oy, kx, ky, vx, vy}.
...
function startRound() {
  G.round++;
  G.level = buildLevel();              // fresh cover/crates each round
  G.projectiles = []; G.pickups = []; G.particles = [];
  const p = G.player;
  p.x = WORLD.w/2; p.y = WORLD.h - 220;
  p.hp = p.maxHp; p.dead = false; p.super = 0;
...
// Called when one duelist is KO'd. Tallies the round and either sets up the
// next round or ends the match.
function resolveRound(winner) {
  if (!G.roundActive || G.roundEndT > 0) return;   // resolve once
  G.roundActive = false;
  if (winner === 'player') { G.playerWins++; showBanner('ROUND WON!', '#38e08a'); }
  else { G.enemyWins++; showBanner('ROUND LOST', '#ff5b5b'); }
  if (G.playerWins >= G.roundsToWin || G.enemyWins >= G.roundsToWin) G.matchOver = true;
  G.roundEndT = 2.2;
  updateHud();
}
...
function endMatch() {
  if (G.over) return;
  G.over = true;
  const won = G.playerWins > G.enemyWins;
...
function update(dt) {
  ...
  if (G.countdown > 0) {
    G.countdown -= dt;
    if (G.countdown <= 0) { G.roundActive = true; showBanner('FIGHT!', '#ffcf3f'); }
  }
  if (G.roundEndT > 0) {
    G.roundEndT -= dt;
    if (G.roundEndT <= 0) {
      if (G.matchOver) { endMatch(); return; }
      startRound();
      return;
    }
  }
  // Combat only runs during the live phase of a round.
  const combat = G.roundActive && G.countdown <= 0 && G.roundEndT <= 0 && !G.matchOver;

  // ---- round timer ----
  if (combat) {
    G.roundTime -= dt;
    if (G.roundTime <= 0) { G.roundTime = 0; resolveByTime(); }
  }

  // ---- player ----
  if (!p.dead && combat) updatePlayer(p, dt);
...
function render() {
  ...
  // touch joysticks (screen space, above the world)
  drawJoysticks();

  // round intro overlay ("ROUND n" + matchup)
  if (G.countdown > 0) {
    ctx.fillStyle = 'rgba(0,0,0,.4)';
    ctx.fillRect(0,0,VW,VH);
    ctx.textAlign = 'center';
    ctx.lineWidth = 6; ctx.strokeStyle = 'rgba(0,0,0,.55)';
    ctx.fillStyle = '#fff'; ctx.font = 'bold 56px "Trebuchet MS", sans-serif';
  ...
  if (G.banner && G.banner.t > 0) {
    const a = clamp(G.banner.t / G.banner.max, 0, 1);
    ctx.globalAlpha = Math.min(1, a * 1.6);
    ctx.textAlign = 'center';
    ctx.font = 'bold 52px "Trebuchet MS", sans-serif';
    ctx.lineWidth = 7; ctx.strokeStyle = 'rgba(0,0,0,.55)';
    ctx.strokeText(G.banner.text, VW/2, VH * 0.32);
    ctx.fillStyle = G.banner.color;
    ctx.fillText(G.banner.text, VW/2, VH * 0.32);
    ctx.globalAlpha = 1;
  }
...
function updateHud() {
  ...
  if (tb) {
    ...
    tb.textContent = Math.floor(s/60) + ':' + String(s%60).padStart(2, '0');
    tb.classList.toggle('urgent', s <= 10 && G.roundActive);
  }
...
/* ----------------------------------------------------------------------
   15. Game over
---------------------------------------------------------------------- */
// (Match end is handled by endMatch(), defined near startRound.)
...
function setupInput() {
  ...
  // ---- touch controls: floating twin-stick ----
  // Drag the LEFT half of the screen to move, the RIGHT half to aim & fire.
  // The stick appears wherever you first press, so there is no tiny target to
  // hunt for — this replaces the old fixed on-screen sticks that overlapped and
  // caused touches to register as "aim" instead of "move" (character stuck).
...
const STICK_R = 62;                 // max knob travel in px
const AIM_DEADZONE = 0.28;          // ignore tiny drags so a tap doesn't fire wildly
...
function onTouchStart(e) {
  for (const t of e.changedTouches) {
    const p = touchLocal(t);
    const zone = p.x < canvas.clientWidth / 2 ? 'move' : 'aim';
    if (input.sticks[zone]) continue;             // one finger owns each zone
    input.sticks[zone] = { id: t.identifier, ox: p.x, oy: p.y, kx: p.x, ky: p.y, vx: 0, vy: 0 };
    applyStick(zone);
  }
  e.preventDefault();
}
function onTouchMove(e) {
  for (const t of e.changedTouches) {
    for (const zone of ['move', 'aim']) {
      ...
      s.vx = dx / STICK_R; s.vy = dy / STICK_R;
      applyStick(zone);
    }
  }
  e.preventDefault();
...
function onTouchEnd(e) {
  for (const t of e.changedTouches) {
    for (const zone of ['move', 'aim']) {
      const s = input.sticks[zone];
      if (s && s.id === t.identifier) { input.sticks[zone] = null; clearStick(zone); }
    }
  }
}
// Push the active stick's vector into the shared input the player logic reads.
function applyStick(zone) {
  const s = input.sticks[zone];
  if (!s) return;
  if (zone === 'move') {
    input.moveVec = { x: s.vx, y: s.vy };
  } else {
    input.aimVec = { x: s.vx, y: s.vy };
    input.aimActive = Math.hypot(s.vx, s.vy) > AIM_DEADZONE;
  }
}
function clearStick(zone) {
  if (zone === 'move') input.moveVec = { x: 0, y: 0 };
  else { input.aimActive = false; input.aimVec = { x: 0, y: 0 }; }
}
// Render the floating joysticks (called from render(), screen space).
function drawJoysticks() {
  for (const zone of ['move', 'aim']) {
    const s = input.sticks[zone];
    if (!s) continue;
    const c = zone === 'move' ? '63,169,255' : '255,77,141';
    ctx.beginPath(); ctx.arc(s.ox, s.oy, STICK_R, 0, TAU);
    ctx.fillStyle = `rgba(${c},0.12)`; ctx.fill();
    ctx.lineWidth = 3; ctx.strokeStyle = `rgba(${c},0.55)`; ctx.stroke();
    ctx.beginPath(); ctx.arc(s.kx, s.ky, STICK_R * 0.46, 0, TAU);
